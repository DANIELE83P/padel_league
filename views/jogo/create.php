...
<?php $header = 'Aggiungi Partita'; ?>
<?php require BASE_PATH . "/views/partials/banner.php"; ?>

<main class="flex-grow">

    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        <a href="/league?id=<?= $_GET['league_id'] ?>"
        ...
        <div class="w-full max-w-sm mx-auto p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
            ...
            <form class="space-y-4 md:space-y-6" method="post">
                <div>
                    ...
                    <label for="local" class="block mb-2 text-sm font-medium text-gray-900">Luogo</label>
                    <input name="local" id="local"
                        value="<?php if (isset($_SESSION['old']['local']))
                            echo $_SESSION['old']['local'] ?>"
                ...
                <?php endif; ?>
                <div>
                    <label for="data_hora" class="block mb-2 text-sm font-medium text-gray-900">Data/Ora</label>
                    <input type="datetime-local" name="data_hora" id="data_hora"
                        value="<?php if (isset($_SESSION['old']['data_hora']))
                            echo $_SESSION['old']['data_hora'] ?>"
                            class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
