    ...
    <main class="flex-grow">
        <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="col-span-4">
                ...
                <?php endif; ?>
                <h1 class="pl-5 block mb-2 text-sm font-medium text-gray-900">Le mie leghe</h1>
                <table class="min-w-full divide-y shadow-md divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr class="text-center">
                            <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Nome della
                                Lega</th>
                            <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Descrizione
                            </th>
                            <th
                                class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider  md:table-cell hidden">
                                Data di Creazione
                            </th>
                            <th
                                class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider  md:table-cell hidden">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($leagues as $league): ?>
                            <tr class="text-center">
                                ...
                                <td class="px-6 py-4 items-center  md:table-cell hidden">
                                    <?= htmlspecialchars((new DateTime($league['data_criacao']))->format("d/m/Y")) ?></td>
                                <td class="px-6 py-4 items-center  md:table-cell hidden">
                                    <?= htmlspecialchars($league['membros_ativos']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div class="bg-white shadow-md rounded-lg p-6 col-span-4 md:col-span-2">
                <h2 class="mb-4 text-lg font-bold">Leghe vicino a te</h2>
            </div>

            <!-- O teu último jogo foi há -->
            <div class="bg-white shadow-md rounded-lg p-6 col-span-4 md:col-span-1 text-center">
                <p class="mb-4 text-lg font-bold">La tua ultima partita è stata:</p>
                <div class="text-4xl font-bold text-blue-600 my-2">
                    <?php if ($daysSinceLastGame == -1) {
                        echo "Nessuna partita.";
                    } else
                        echo $daysSinceLastGame ?></div>
                <?php if ($daysSinceLastGame == 1): ?>
                    <p class="text-gray-500">giorno fa</p>
                <?php elseif ($daysSinceLastGame > 1): ?>
                    <p class="text-gray-500">giorni fa</p>
                <?php endif; ?>
            </div>

            <!-- Últimos jogos -->
            <div class="bg-white shadow-md rounded-lg p-6 col-span-4 md:col-span-1 overflow-y-auto">
                <h2 class="mb-4 text-lg font-bold">Ultime partite:</h2>
                    ...
                    <a href="game?id=<?= $game['id'] ?>" class="block mb-4">
                        <div class="p-4 rounded-lg shadow-sm border border-gray-200">
                            <div class="flex items-center">
                                <div
                                    class="<?= $game['Resultado'] == 'Vittoria' ? 'bg-green-500' : 'bg-red-500' ?> w-4 h-4 rounded-full mr-2">
                                </div>
                                <span
                                    class="font-bold text-sm"><?= $game['Resultado'] == 'Vittoria' ? 'Vittoria' : 'Sconfitta' ?>
                                </span>
                                <span class="ml-auto text-sm text-gray-600"><?= $game['team1_score'] ?> -
                                    <?= $game['team2_score'] ?></span>
                            </div>
                            <p class="mt-2 text-sm text-gray-600">Luogo: <?= $game['local'] ?></p>
