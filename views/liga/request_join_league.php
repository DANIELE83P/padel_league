...
<?php $header = 'Richiesta di Adesione'; ?>
    ...
    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        ...
        <div class="w-full max-w-sm mx-auto p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
            ...
            <div class="mb-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <p class="text-xs text-gray-400">Creato il:
                    <?= htmlspecialchars((new DateTime($leagueData['data_criacao']))->format("d/m/Y")) ?></p>
            </div>

            <form class="space-y-4 md:space-y-6" method="post">
                <div>
                    <label for="message" class="block mb-2 text-sm font-medium text-gray-900">Scrivi un messaggio per
                        unirti</label>
                    <input name="message" id="message"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder="Messaggio" required="">
                </div>
