...
<main class="flex-grow">
    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        ...
        <div class="w-full max-w-sm mx-auto p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8">
            <h2 class="text-2xl font-bold mb-4">Opzioni della Lega</h2>
            <form method="post">
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="tipo_liga">
                        Visibilità della Lega:
                    </label>
                    <select
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        id="tipo_liga" name="tipo_liga">
                        <option <?php if ($leagueDetails['tipo_liga'] == 'publica')
                            echo 'selected' ?> value="publica">
                                Pubblico</option>
                            <option <?php if ($leagueDetails['tipo_liga'] == 'privada')
                            echo 'selected' ?> value="privada">
                                Privato</option>
                        </select>
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2" for="description">
                            Descrizione della Lega:
                        </label>
                        <textarea
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                ...
                <div class="flex flex-col items-center justify-between space-y-4">
                    ...
                    <a href="/league/confirm-delete?id=<?php SessionController::leagueForDeletion($leagueDetails['id']);
                    echo $leagueDetails['id']; ?>"
